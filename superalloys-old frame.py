...
from definition import models,param_space_old_frame
from functions import calculate_ABD_I, objective_superalloy_old
...
if __name__ == '__main__':
    ...
    best_val_mape_progress = {}
    best_val_r2_progress = {}
    best_train_mape_progress = {}
    best_train_r2_progress = {}

    best_val_mape_std_progress = {}
    best_val_r2_std_progress = {}
    best_train_mape_std_progress = {}
    best_train_r2_std_progress = {}
    ...
    for model_name in models.keys():
        ...
        study.optimize(lambda trial:
                       objective_superalloy_old(trial, models, model_name, X_train, y_train, train_groups, param_space_old_frame),
                       n_trials=100)
        ...
        best_trial_index = -1
        for i in range(len(val_mape_averages)):
            if val_mape_averages[i] < best_mape_so_far:
                best_mape_so_far = val_mape_averages[i]
                best_trial_index = i


            best_val_mape_up_to_trial.append(best_mape_so_far)
        ...
        print(f"{model_name} - Final best val MAPE: {best_mape_so_far:.4f}")
        print(f"{model_name} - Final best val R2: {val_r2_averages[best_trial_index]:.4f}")
        print(f"{model_name} - Final best val MAPE: {train_mape_averages[best_trial_index]:.4f}")
        print(f"{model_name} - Final best val R2: {train_r2_averages[best_trial_index]:.4f}")
    ...
    # ===============================
    acquisition_functions = ['EI', 'PI', 'UCB', 'Greedy', 'Pure exploration', 'Ideal']
    ...
    for model_name, base_model in models.items():
        ...
        for af in acquisition_functions:
            ...
            selected_dict[model_name][af] = []
            selected_pre[model_name][af] = []
            selected_pre_std[model_name][af] = []
            true_mu_per_step[model_name][af] = []
            pre_mu_per_step[model_name][af] = []
            pre_std_per_step[model_name][af] = []
            ABD_I[model_name][af] = []
            ...
            while len(X_pool_active) > 0:
                ...
                print(f"Step {iter_round}: Train MAPE = {train_mape:.4f}, 样本数 = {len(X_train_active)}")
                ...
                true_mu_per_step[model_name][af].append(val_cte_active.tolist())
                pre_mu_per_step[model_name][af].append(mu.tolist())
                pre_std_per_step[model_name][af].append(sigma.tolist())

                # Calculate score with different acquisition functions
                if af == 'EI':
                    y_best = np.min(y_train_active)
                    Z = (y_best - mu) / (sigma + 1e-8)
                    scores = -((y_best - mu) * norm.cdf(Z) + sigma * norm.pdf(Z))
                elif af == 'PI':
                    y_best = np.min(y_train_active)
                    Z = (y_best - mu) / (sigma + 1e-8)
                    scores = -norm.cdf(Z)
                    ...
                    scores = mu
                elif af == 'Pure exploration':
                    scores = -sigma
                elif af == 'Ideal':
                    scores = val_cte_active
                ...
                # Record Top-K
                selected_dict[model_name][af].append(selected_cte.tolist())
                selected_pre[model_name][af].append(y_pre.tolist())
                selected_pre_std[model_name][af].append(y_std.tolist())
            ...
            # Save
            learning_curves[model_name][af] = step_losses
            ABD_I[model_name][af] = calculate_ABD_I(selected_dict[model_name][af])
        print(f"✅ {model_name}  finished with all acquisition functions.")
